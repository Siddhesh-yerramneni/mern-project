  ...
  return (
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
        ...
        <img onClick={()=>fileRef.current.click()} src={formData.avatar || currentUser.avatar} alt="Profile image" className='self-center mt-2 rounded-full h-24 w-24 object-cover cursor-pointer'/>
        ...
        <button disable={loading} className='bg-slate-700 text-white rounded-lg p-3 uppercase hover:opacity-95 disabled:opacity-80'>{loading ? 'loading...' : 'Update'}</button>
    ...
    {userListings.map((listing) => (
      <div key={listing._id} className="border rounded-lg flex justify-between items-center p-3">
        ...
        <div className="flex flex-col items-center">
          <button onClick={()=>handleDeleteListing(listing._id)} type='button' className="text-red-700 uppercase p-1">
            Delete
          </button>
          <Link to={`/update-listing/${listing._id}`}><button type='button' className="text-green-700 uppercase  p-1">
            Edit
          </button></Link>
